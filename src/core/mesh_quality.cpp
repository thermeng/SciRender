...
MeshQuality analyzeMeshQuality(const RenderMesh& mesh) {
    MeshQuality q;
    mesh.ensureFlatVerts();
    const auto& fv = mesh.flatVerts;
    ...
    struct DirectedEdge {
        ...
        uint32_t u;
        uint32_t v;
        bool operator<(const DirectedEdge& o) const { return key < o.key; }
    };
    ...
    for (size_t t = 0; t < nt; ++t) {
        if (isDegen[t]) continue;
        uint32_t corners_[3] = { weld[t * 3], weld[t * 3 + 1], weld[t * 3 + 2] };
        vertSurvivingTris[corners_[0]].push_back(static_cast<uint32_t>(t));
        vertSurvivingTris[corners_[1]].push_back(static_cast<uint32_t>(t));
        vertSurvivingTris[corners_[2]].push_back(static_cast<uint32_t>(t));
        for (int i = 0; i < 3; ++i) {
            uint32_t u = corners_[i];
            uint32_t v = corners_[(i + 1) % 3];
            uint32_t x = u < v ? u : v;
            uint32_t y = u < v ? v : u;
    ...
    while (edgeIdx < numFlat) {
        ...
        while (runEnd < numFlat && flatEdges[runEnd].key == flatEdges[edgeIdx].key) runEnd++;
    ...
    for (uint32_t s = 0; s < nv; ++s) {
        ...
        while (!linkStack.empty()) {
            ...
            for (uint32_t nxt : link[curr]) {
                if (!linkVis[nxt]) { linkVis[nxt] = 1; linkStack.push_back(nxt); }
            }
        }
        if (reached < numNb) q.nonManifoldVerts++;
    }
